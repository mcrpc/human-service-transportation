...
acsVariableTibble <- tibble::tribble(
  ...
  "est_pop", "Population Estimate", est_pop <- c("B01001_001E"), NULL,
  "est_blw18", "Population Under 18 Estimate", est_blw18 <- paste0("B01001_0", stringr::str_pad(c(3:6,27:30), 2, "left", pad = "0"), "E"), dnm_blw18 <- "B01001_001E",
  "est_18to65", "Population Between 18 and 65 Estimate", est_18to65 <- paste0("B01001_0", stringr::str_pad(c(7:9,10:19,31:43), 2, "left", pad = "0"), "E"), dnm_18to65 <- "B01001_001E",
  "est_ovr65", "est_ovr65", est_ovr65 <- paste0("B01001_0", c(20:25, 44:49), "E"), dnm_ovr65 <- "B01001_001E",
  "est_vet", "est_vet", est_vet <- "B21001_002E", dnm_vet <- "B21001_001E",
  "est_dsblty", "est_dsblty", est_dsblty <- subset(acs2017VariableTable, stringr::str_detect(name, "B18101_") & stringr::str_detect(label, "With a disability")) %>% .$name %>% paste(.,"E",sep=""), dnm_dsblty <- "B18101_001E",
  "est_blwpov", "est_blwpov", est_blwpov <- "B17020_002E", dnm_blwpov <- "B17020_001E",
  "est_nocars", "est_nocars", est_nocars <- "B08141_002E", dnm_nocars <- "B08141_001E",
  "gini", "GINI Coefficient", gini <- "B19083_001E", NULL,
  "est_nodipl", "est_nodipl", est_nodipl <- paste0("B15003_0", stringr::str_pad(c(2:16), 2, "left", pad = "0"), "E"), dnm_nodipl <- "B15003_001E",
  "inc_percap", "inc_percap", inc_percap <- "B19301_001E", NULL,
  "inc_medhh", "inc_medhh", inc_medhh <- "B22008_001E", NULL,
  "est_hhSo65", "est_hhSo65", est_hhSo65 <- "B22001_003E", dnm_hhSo65 <- "B22001_001E"
)
...
tryCatch(
  {
    illinoisTractData <- illinoisTractData %>%
      dplyr::transmute(
        ...
        dnm_hhSo65 = rowSums(.[names(.) %in% dnm_hhSo65])
      )
  },
  error = function(err) {
